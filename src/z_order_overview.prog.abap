*&---------------------------------------------------------------------*
*& Report Z_BEST_UEBERSICHT_CL
*&---------------------------------------------------------------------*
*&
*&---------------------------------------------------------------------*
REPORT z_order_overview.

DATA: lv_filter    TYPE         i.
DATA: lv_object TYPE balobj_d VALUE 'ZWEB'.
DATA: lv_subobject TYPE balobj_d VALUE 'ZWEB'.

PARAMETERS: p_rkunde RADIOBUTTON GROUP rad1      USER-COMMAND test
            ,p_kundid TYPE zweb_kd_numr_de        MODIF ID kid  MATCHCODE OBJECT zweb_sh_customer.

PARAMETERS: p_rbest RADIOBUTTON GROUP rad1
            ,p_best   TYPE zweb_order_number  MODIF ID bes  MATCHCODE OBJECT zweb_sh_ordernumber. "=>Mit Suchhilfen Exit löscht doppelte Einträge

PARAMETERS: p_rstat  RADIOBUTTON GROUP rad1
            ,p_status TYPE zweb_status         MODIF ID sta.
...
AT SELECTION-SCREEN.
  ...
  CASE sy-ucomm.
    WHEN 'SEARCH'.
      ...
      TRY.
          DATA(lo_log) = NEW zcl_webshop_log( iv_object = lv_object iv_suobj = lv_subobject ).
          DATA(lo_start) = NEW zcl_order_overview_controller( iv_filter        = lv_filter
        ...
        CATCH zcx_webshop_exception_new INTO DATA(e_txt).
